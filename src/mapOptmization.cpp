...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void updatePath(const PointTypePose& pose_in)
  {
    ...
    /* save keyframe poses only */
    // if (saveTUMPose)
    // {
    //   std::ofstream ofs;
    //   ofs.open("/home/sam/liosam_ws/src/LIO-SAM-Modified/globalPath.txt", std::ios::app);
    //   Eigen::Quaterniond q_L = Eigen::Quaterniond(q.w(), q.x(), q.y(), q.z());
    //   Eigen::Vector3d t_L = Eigen::Vector3d(pose_in.x, pose_in.y, pose_in.z);
    //   q_L = extQRPY.inverse()*q_L*extQRPY;
    //   t_L = extTrans+extQRPY.inverse()*t_L-q_L*extTrans;
    //   ofs<<std::setprecision(18)<<pose_in.time<<" "<<std::setprecision(12)
    //     <<t_L(0)<<" "<<t_L(1)<<" "<<t_L(2)<<" "
    //     <<q_L.x()<<" "<<q_L.y()<<" "<<q_L.z()<<" "<<q_L.w()<<std::endl;
    //   ofs.close();
    // }
  }
  ...
  void publishFrames()
  {
    ...
    if (pubPath.getNumSubscribers()!=0)
    {
      ...
      {
        std::ofstream ofs;
        ofs.open(savePCDDirectory+"globalPath.txt", std::ios::app);
        PointTypePose thisPose6D = trans2PointTypePose(transformTobeMapped);
        tf::Quaternion q = tf::createQuaternionFromRPY(thisPose6D.roll, thisPose6D.pitch, thisPose6D.yaw);
        Eigen::Quaterniond q_L = Eigen::Quaterniond(q.w(), q.x(), q.y(), q.z());
        Eigen::Vector3d t_L = Eigen::Vector3d(thisPose6D.x, thisPose6D.y, thisPose6D.z);
        q_L = extQRPY.inverse()*q_L*extQRPY;
        t_L = extTrans+extQRPY.inverse()*t_L-q_L*extTrans;
        ofs<<std::setprecision(18)<<timeLaserInfoStamp.toSec()<<" "<<std::setprecision(12)
          <<t_L(0)<<" "<<t_L(1)<<" "<<t_L(2)<<" "
          <<q_L.x()<<" "<<q_L.y()<<" "<<q_L.z()<<" "<<q_L.w()<<std::endl;
        ofs.close();
      }
